...
module.exports = function(opts) {
  var css = fs.readFileSync(__dirname + '/bespoke-overview.css', 'utf8');
  ...
  return function(deck) {
    ...
    activateOverview = function() {
      ...
      deck.slides.forEach(function(slide) {
        ...
        // NOTE drop exponential notation in near-zero numbers (breaks older WebKit engines)
        if (x.toString().toLowerCase().indexOf('e') !== -1) x = x.toFixed(12);
        if (y.toString().toLowerCase().indexOf('e') !== -1) y = y.toFixed(12);
        // TODO use CSS prefixes for -webkit and maybe -ms
