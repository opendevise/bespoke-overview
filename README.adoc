...
This plugin adds an overview mode to a http://markdalgleish.com/projects/bespoke.js[Bespoke.js] presentation, which arranges all the slides in the deck on a grid when the kbd:[o] key is pressed.
...
This configuration ensures that both the scale and overview plugins work in Internet Explorer 10 and above (as well as Opera/Presto). _IE 9 and below are not supported by Bespoke.js._
...
bespoke.from('.deck', [
  // ...
  scale('webkitAppearance' in html.style ? 'zoom' : 'transform'),
  overview();
]);
```
...
bespoke.from('.deck', [
  // ...
  bespoke.plugins.scale('webkitAppearance' in html.style ? 'zoom' : 'transform'),
  bespoke.plugins.overview();
]);
```
